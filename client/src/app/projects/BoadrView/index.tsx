import { Priority, Status, useGetTasksQuery, useUpdateTaskMutation } from '@/state/api';
import React, { use } from 'react'
import { DndProvider, useDrag, useDrop } from 'react-dnd';
import { HTML5Backend } from 'react-dnd-html5-backend';
import { skipToken } from '@reduxjs/toolkit/query/react';
import { Task as TaskType, Attachment } from '@/state/api';
import { EllipsisVertical, EllipsisVerticalIcon, MessageSquareMore, Plus } from 'lucide-react';
        ...
        >
            ...
            />
                    <div className='flex w-full items-center justify-between rounded-e-lg bg-white  px-5 py-4 dark:bg-dark-secondary'>
                        <h3 className='flex items-center text-md font-bold dark:text-white'>
                            {status}{" "}
                            <span
                            className='ml-2 inline-block rounded-full bg-gray-200 
...
type TaskProps = {
    task: TaskType & { attachments?: Attachment[] }
}
        ...
        >
            ...
            <div className='p-4 md:p-6'>
                <div className='flex items-start justify-between'>
                    <div className='flex flex-1 flex-wrap items-center gap-2'>
                        ...
                        <div className='flex gap-2'>
                            {taskTagsSplit.map((tag)=>(
                                <div key={tag} className='rounded-full font-semibold bg-blue-100 px-2 py-1 text-xs'>
                                    {" "}
                                    {tag}
                                </div>
                            ))}
                    ...
                    <button className='flex h6- w-4 flex-shrink-0 items-center justify-center dark:text-neutral-500'>
                        <EllipsisVertical size={26} />
                    </button>
                </div>
                    ...
                    <div className='flex -space-x-[6px] overflow-hidden'>
                        {task.assignee && (
                            <Image 
                                ...
                                src={`/icons/${task?.assignee?.profilePictureUrl!}`}
                        ...
                        {task.author && (
                            <Image 
                                ...
                                src={`/icons/${task?.author?.profilePictureUrl!}`}
